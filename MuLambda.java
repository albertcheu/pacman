...
public class MuLambda extends Controller<MOVE>{
    private MOVE[] allMoves = MOVE.values();
    private Legacy lg = new Legacy();
    private int C = 4;
    private int MU = 12;
    private int LAMBDA = 14;
	...
	/*
	  Generate MU+LAMBDA random sequences of C integers between 0 and 5, inclusive
	  For each, progress copy of state:
	  ...
	  Until out of time,
  	    Sort the MU+LAMBDA sequences
	    Replace the LAMBDA worst with random changes to the MU best sequences
	    For each sequence, progress copy of game state and evaluate
	...
	*/
	...
	class Individual{
	    ...
	    Individual(MOVE m, int i, ArrayList<Integer> al){
		firstMove = m;
		score = i;
		choices = al;
	    }
	...
	class CustomComparator implements Comparator<Individual>{
		...
		//descending order: return how much bigger b is than a
		if (a.score != Integer.MIN_VALUE && b.score != Integer.MIN_VALUE)
		    { return b.score-a.score; }

		//If a is MIN_VALUE,
		if (a.score == Integer.MIN_VALUE) { return Integer.MAX_VALUE; }
	...
	for (int i = 0; i < MU+LAMBDA; i++){
	    ...
	    int s = score(copy);
	    population.add(new Individual(firstMove,s,choices));
	}
	...
	while (timeDue-4 > System.currentTimeMillis()){
		...
		MOVE firstMove = progress(choices, copy);
		int s = score(copy);
		population.set(MU+i,new Individual(firstMove,s,choices));
	    }
	}
